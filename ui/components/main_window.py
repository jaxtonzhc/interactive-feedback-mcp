...
class FeedbackUI(QMainWindow):
    ...
    def _setup_window(self):
        ...
        self.setWindowTitle(f"Cursor 交互式反馈 MCP  ⏰ {ts}")
    ...
    def _create_ui(self):
        ...
        # 醒目的时间戳横幅，帮助区分多个弹窗
        self._create_timestamp_banner(layout)
    ...
    def _create_timestamp_banner(self, layout):
        """在顶部创建醒目的时间戳横幅，方便区分多个弹窗"""
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        banner = QLabel(f"🕐 弹出时间: {ts}")
